...
import { libertyAI } from '../agents/liberty-ai'
...
export function createManualToolStreamResponse(config: BaseStreamConfig) {
  return createDataStreamResponse({
    execute: async (dataStream: DataStreamWriter) => {
      const { messages, model, chatId, searchMode, userId, chatMode } = config
      ...
      try {
        ...
        // Use Liberty AI for general mode, otherwise use manual researcher
        const researcherConfig = chatMode === 'general' 
          ? libertyAI({
