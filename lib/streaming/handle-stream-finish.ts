...
import { generateRelatedQuestions, generateRelatedQuestionsStream } from '@/lib/agents/generate-related-questions'
...
export async function handleStreamFinish({
  ...
  try {
    ...
    if (!skipRelatedQuestions) {
      ...
      // Generate related questions with streaming
      const relatedQuestionsStream = await generateRelatedQuestionsStream(
        responseMessages,
        model
      )
      ...
      for await (const partialObject of relatedQuestionsStream.partialObjectStream) {
        if (partialObject && partialObject.items) {
          ...
          dataStream.writeMessageAnnotation(streamingAnnotation)
        }
      }

      // Get the final result
      const relatedQuestions = await relatedQuestionsStream.object

      // Create and add final related questions annotation
